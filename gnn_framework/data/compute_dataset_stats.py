...
def compute_dataset_stats(train_loader, device, avoid_zero_div=False):
    ...
    with torch.no_grad():
        ...
        glob_sum = torch.zeros(base_dataset.num_glob_features, requires_grad=False)

        # glob_sum = torch.zeros(base_dataset.num_glob_features, requires_grad=False)
        glob_sum_squared = torch.zeros(base_dataset.num_glob_features, requires_grad=False)

        # iterative variables for the edge attributes
        ea_min = []
        ea_max = []
        ea_sum = torch.zeros(base_dataset.num_edge_features, requires_grad=False)
        ea_sum_squared = torch.zeros(base_dataset.num_edge_features, requires_grad=False)

        # iterative variables for the y values
        y_min = []
        y_max = []
        y_sum = torch.zeros(base_dataset.num_node_output_features, requires_grad=False)
        y_sum_squared = torch.zeros(base_dataset.num_node_output_features, requires_grad=False)

        if base_dataset.num_node_features != 0:
            # iterative variables for the x values
            x_min = []
            x_max = []
            x_sum = torch.zeros(base_dataset.num_node_features, requires_grad=False)
            x_sum_squared = torch.zeros(base_dataset.num_node_features, requires_grad=False)
